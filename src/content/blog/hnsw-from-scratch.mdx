...
Below is an illustration of KNN, where we have a vector "S" with value of [2,3]. We want to find the closest vector (`k` = 1) in the set:
...
We can see that the vector "S" is closest to vector 2, so the KNN algorithm would return vector 2 as the nearest neighbor.
